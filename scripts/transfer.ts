...
programCommand('transfer')
  ...
  .option(
    '-s, --symbol <string>',
    'Symbol to filter NFTs for transfer',
    'CC'
  )
  .action(async (toKey, options, cmd) => {
    ...
    const tokenAccounts = await Promise.all(resp.value.map(async ({account, pubkey}) => {
      ...
      if (metadataAccount) {
        metaData = await programs.metadata.Metadata.load(connection, metadataAccount.pubkey);
      }
      return {tokenAccount, metaData}
    }));

    // Filter all owned NFTs by symbol
    const nftAccounts = tokenAccounts.filter(x => x.metaData && x.metaData.data.data.symbol === symbol);

    // Initiate the transfer for each NFT
    console.log(`Starting transfer of ${nftAccounts.length} NFTs with symbol ${symbol} to ${toKey.toString()}`)

    await Promise.all(nftAccounts.map(async (nftAccount) => {
